...
export const metadata: Metadata = {
  title: "Vidyut Team | VishwaDev",
  description: "Building the future, one project at a time. The official Vidyut developer squad.",
};

export default function VidyutTeamPage() {
  const team = teams.find((t) => t.id === "vidyut");

  if (!team) {
    notFound();
  }

  // Cool fun stats for Vidyut
  const funStats = [
    { label: "Elite Devs", value: "4", icon: Users, desc: "Full-stack powerhouses" },
    { label: "Deployed Apps", value: "3", icon: Folder, desc: "Pristine production web-apps" },
    { label: "LOC Shipped", value: "500K+", icon: Code2, desc: "Clean TypeScript architectures" },
    { label: "Coffees Consumed", value: "2.4K+", icon: Coffee, desc: "Fueling our code engines" }
  ];

  const coreValues = [
    { title: "SHIPPED > PERFECT", desc: "Build fast, iterate aggressively, deliver real-world value." },
    { title: "HIGH VOLTAGE STACK", desc: "We master React, NextJS, Node, and IoT integrations." },
    { title: "COLLABORATIVE FORCE", desc: "Single-source cohesion. We review, pair-program, and ship as one." }
  ];

  return (
    <div className="min-h-screen bg-background">
      {/* High-Voltage Accent Top Bar */}
      <div className="h-2 w-full bg-[#f59e0b]" />
      ...
      <div className="container mx-auto px-4 pt-20 sm:pt-24 lg:pt-28 pb-16">
        ...
        <div className="max-w-6xl mx-auto mb-8 flex items-center justify-between">
          ...
          <Badge className="bg-[#f59e0b] hover:bg-[#f59e0b] text-black border-2 border-black font-black uppercase text-xs shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] py-1 px-3">
            ⚡ SQUAD STATUS: FULLY CHARGED
          </Badge>
        </div>
        ...
        <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8 mb-12">
          ...
          <div className="lg:col-span-7 bg-white border-[3px] border-black shadow-[8px_8px_0px_0px_rgba(245,158,11,1)] rounded-xl overflow-hidden group">
            <div className="relative h-72 sm:h-96 w-full bg-muted">
              <Image 
                src="/teams/vidyut_team_cover.png"
                alt="Vidyut Team Cover"
                fill
                priority
          ...
          <div className="lg:col-span-5 bg-white border-[3px] border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] rounded-xl p-6 sm:p-8 flex flex-col justify-between">
            <div>
              ...
              <div className="space-y-3.5">
                ...
                <div className="flex justify-between items-center border-b border-dashed border-black/10 pb-2.5">
                  <span className="text-xs font-black uppercase text-muted-foreground">Focus Domain</span>
                  <span className="text-sm font-bold text-black bg-[#fafafa] border border-black px-2 py-0.5 rounded shadow-[1px_1px_0px_0px_rgba(0,0,0,1)]">
                    Full-Stack & IoT Systems
                  </span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-xs font-black uppercase text-muted-foreground">Current Task</span>
                  <span className="text-xs font-black text-[#f59e0b] animate-pulse">
                    🟢 ONLINE & COMMITTING
                  </span>
                </div>
              </div>
            ...
            <div className="mt-8">
              <Button variant="default" size="lg" className="w-full font-bold border-2 border-black shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] hover:bg-[#f59e0b] hover:text-black hover:scale-[1.01] transition-all text-sm py-4" asChild>
                <a href="mailto:info@vishwadev.tech?subject=Inquiry%20for%20Team%20Vidyut">
                  Hire Team Vidyut
                </a>
              </Button>
            </div>
          ...
          {funStats.map((stat, idx) => (
            <div key={idx} className="bg-white border-[3px] border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] rounded-xl p-5 hover:scale-102 transition-transform duration-200">
              <div className="flex justify-between items-start mb-3">
                <div className="p-2 bg-[#f59e0b]/10 border border-black rounded-lg">
        ...
        <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8 mb-12">
          ...
          <div className="lg:col-span-7 space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {team.members.map((member) => (
                  <Link
                    ...
                    className="group flex flex-col justify-between bg-[#fafafa] hover:bg-white border-2 border-black hover:border-black rounded-lg p-4 transition-all shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] hover:shadow-[4px_4px_0px_0px_rgba(245,158,11,1)] hover:scale-102"
                  >
            ...
            <div className="bg-black text-green-400 border-[3px] border-black shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] rounded-xl p-6 font-mono text-xs">
              <div className="flex justify-between items-center border-b border-green-400/20 pb-3 mb-4">
                <span className="flex items-center gap-1.5">
                  <span className="w-2.5 h-2.5 rounded-full bg-red-500 inline-block"></span>
                  <span className="w-2.5 h-2.5 rounded-full bg-yellow-500 inline-block"></span>
                  <span className="w-2.5 h-2.5 rounded-full bg-green-500 inline-block"></span>
                  <span className="text-green-400 font-bold ml-2">vidyut-core-protocol.sh</span>
                </span>
              ...
              <div className="space-y-4">
                {coreValues.map((val, idx) => (
                  <div key={idx} className="border-l-2 border-green-500/30 pl-3">
                    <p className="font-bold text-[#f59e0b]">$ {val.title}</p>
                    <p className="text-green-300/80 mt-1 leading-relaxed">{val.desc}</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="lg:col-span-5 space-y-6">
            <div className="bg-white border-[3px] border-black shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] rounded-xl p-6 sm:p-8">
              <div className="flex items-center gap-2.5 mb-6">
                ...
                <h3 className="font-black text-sm text-foreground uppercase tracking-widest">
                  Flagship Launchpad ({team.projects.length})
                </h3>
              </div>
              ...
              <div className="space-y-4">
                {team.projects.map((project, index) => (
                  <a
                    ...
                    className="flex flex-col justify-between p-4 bg-[#fafafa] hover:bg-white border-2 border-black rounded-lg transition-all group shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] hover:shadow-[4px_4px_0px_0px_rgba(245,158,11,1)] hover:scale-102"
                  >
                    ...
                    <div className="flex justify-between items-center border-t border-dashed border-black/10 pt-2.5 mt-2 text-[10px]">
                      ...
                      <Badge className="bg-[#ffde47] hover:bg-[#ffde47] text-black border border-black font-black text-[9px] py-0.5 shadow-[1px_1px_0px_0px_rgba(0,0,0,1)]">
                        {index === 0 ? "Production" : index === 1 ? "Active IoT" : "Squad Core"}
                      </Badge>
                    </div>
                  </a>
                ))}
            ...
            <div className="bg-white border-[3px] border-black shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] rounded-xl p-6">
              <div className="flex items-center gap-2 mb-3">
                ...
                <h4 className="font-black text-xs uppercase tracking-widest text-foreground">SQUAD ACHIEVEMENT</h4>
              </div>
              <div className="bg-[#fafafa] border-2 border-black rounded-lg p-4 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]">
                <p className="text-sm font-black text-[#f59e0b] mb-1">🏅 HACKATHON SHIELD WINNERS</p>
                <p className="text-xs text-black/70 font-semibold leading-relaxed">
                  Shipped the complete Haazri IoT interface inside a contiguous 36-hour sprint, claiming first prize in hardware integration workflows.
                </p>
              </div>
            </div>
